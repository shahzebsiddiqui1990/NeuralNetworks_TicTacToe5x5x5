	...
	public:	
		...
		void UpdateWeights()
		{
			...
			for (int i = 0; i < NUM_INPUT_NEURONS; i++)
			{
				...
				{
					//cout << "inputToHiddenWeights["<<i<<"]["<<j<<"] : "<< inputToHiddenWeights[i][j] << endl;
			...
			for (int i = 0; i < NUM_HIDDEN_NEURONS; i++)
			{
				...
				{
					//cout << "hiddenToOutputWeights["<<i<<"]["<<j<<"] : "<<hiddenToOutputWeights[i][j]<< endl;
		...
		void SelectPlayer(ifstream & file1, ifstream & file2)
		{
			...
			if (!file1 || !file2)
			{
				if(!file1)
				{
					cout<<"Error in SelectPlayer: Missing Before File"<<endl;
				}
				else if(!file2)
				{
					cout<<"Error in SelectPlayer: Missing Desired File"<<endl;
				}
				else
				{
